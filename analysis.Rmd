...
rum_best_model <- emmeans::emmip(best_model_grid, 
                                  formula = rumination ~ Timepoint | Appraisal,
                                  CIs = TRUE, 
                                  frequentist = TRUE) +
  ggplot2::scale_x_discrete(labels = c("Post Induction", "Post Listening")) + 
...
This doesn't evaluate as it is very time consuming. The contrasts in a tibble have been saved along with the raw emmeans returned value. 
```{reval=FALSE}

best_model_contrast <- emmeans::contrast(best_model_grid, 
                                         method = "pairwise")

write_rds(best_model_contrast, 
          here::here("data", "exploratroy_contrasts.Rds"))

tidy_contrast <- broom::tidy(best_model_contrast)

write_csv(tidy_contrast, 
          here::here("data", "exploratory_contrasts_tidy.csv"))
```

This code chunk starts by importing the previously exported tibble of contrasts 
```{r EvalContrasts}

relevant_contrasts <- tidy_contrast %>% 
...
relevant_contrasts %>% 
  filter(Appraisal == "Yes") %>% 
  select(Timepoint, Timepoint2, Contagion, Appraisal, Conditioning, Memory, rumination, baseline, estimate, p.value) %>% 
  view()
...
Here I set up a hypothetical best case scenerio in the reference grid. 
The results indicate that under best case scenarios the effect cannot be reversed. Although it is nullified. 
```{r}
best_case_grid <- emmeans::ref_grid(mod4, 
                  at = list(rumination = c(minus_rum, mean_rum, plus_rum), 
                            Baseline = c(minus_baseline, mean_baseline, plus_baseline), 
                            Contagion = "No", 
                            Appraisal = "Yes", 
...
best_case_contrast <- emmeans::contrast(best_case_grid, 
                                        "pairwise") %>% 
  broom::tidy() %>% 
  tidyr::separate(col = level1, 
                  into = c("Timepoint", "rumination", "baseline", "Contagion", "Appraisal", "Conditioning", "Memory"),
